...
我们将使用一个在 ImageNet 上预训练好的 ResNet-18 或 VGG-16，来解决 CIFAR-10（或你自定义的小数据集）分类问题。
...
4. LSTM 是 CV 领域的常用模型，为什么这里也能使用 LSTM 进行时间序列预测？
5. Transformer 的自注意力机制在时间序列预测中有何优势？
6. 在当前数据集下，Transformer 是否优于 RNN/LSTM？

然后阅读以下内容，再次思考以下问题（题目原文放在最后）：
...
>
> 只看上面的摘要会给人一种错觉，认为这是一道明确的物理建模题目，但是事实上如果你阅读完整题目，你会发现这道题目其实并不适合完全地使用传统物理建模方法来解决。
>
> 因为这道题最后是要求使用 5 架无人机，每架无人机最多投放 3 枚烟幕干扰弹，实施对 3 枚来袭导弹的干扰。这个规模已经远远超出了传统物理建模方法的适用范围。
...
明确的物理规律：烟幕弹在重力作用下运动，起爆后形成半径 10m 的云团，以 3m/s 速度匀速下沉。

明确的几何关系：导弹位置、无人机位置、保护目标位置已知。

明确的目标：通过调整投放时间、速度和角度，使得烟幕遮蔽时间最大化。
