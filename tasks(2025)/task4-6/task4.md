...
我们后续主要探索更前沿的深度学习，这部分内容只需要浅尝辄止，不需要深入研究其内部复杂的实现原理。

重点是了解其功能，并在未来需要时，能够熟练地查阅官方文档，直接调用其提供的强大接口来解决问题。
...
完成 [CS231n Assignment 1](https://cs231n.github.io/assignments2025/assignment1/)
...
本次作业将引导你面对一个真实的预测任务，你将不再使用简单的示例数据集，而是要处理带有缺失值和噪音的原始数据，最终提交你的预测结果，体验一次完整的数据科学竞赛之旅。

#### 数据探索与预处理

在这一部分，你要对数据进行全面的探索和分析，清洗和整理泰坦尼克号的乘客数据，为后续的模型训练做好准备。
