...
对于基于树的复杂集成算法（如随机森林、XGBoost、决策树），由于其实现逻辑与神经网络差异较大，只需要了解原理并掌握 [scikit-learn](https://scikit-learn.org/stable/) 即可，无需从零实现。但对于线性分类器（k-NN, SVM, Softmax），由于它们是理解神经网络损失函数和梯度下降的基石，你需要在作业 1 中亲手实现。
...
注意，该数据集包含大量天空拍摄图像。作业仅要求基于历史功率数据进行预测。请忽略图像文件，只下载数据集的 CSV 文件，并从其中提取时间与功率列进行训练。

不要尝试将原始图像直接输入到 SVR 或 XGBoost 中。
