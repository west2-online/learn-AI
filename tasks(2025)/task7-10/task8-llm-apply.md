...
1. 创建并注册至少 3 个工具：
    1. 工具 1（核心）：领域知识库（Custom Tool）：将阶段二构建的高级 RAG 链封装成一个 LangChain `Tool`。当 Agent 需要回答关于爬取领域的内部知识时，它应该调用这个工具
    2. 工具 2（扩展）：实时网络搜索引擎（Built-in Tool）：集成一个 LangChain 的内置工具，如 `SerperAPIWrapper` 或 `DuckGoSearchRun`。当 Agent 发现领域知识库无法回答时，它应该调用这个工具
    3. 工具 3（功能）：计算器或 Python REPL（Built-in Tool）：集成 `PythonREPLTool` 或 `LLMMathChain`。当 Agent 需要进行数学计算时，它应该调用这个工具
...
2. 报告中需详细说明：
    1. 项目设计、爬虫目标网站和数据处理流程
    2. 微调思路、数据集和微调后的效果对比（如果进行了微调）
    3. 高级 RAG 实现了哪种优化，效果如何
    4. Agent 的设计方案，每个工具的作用是什么
