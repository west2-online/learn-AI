...
class Character:
    ...
    def give_gift(self, gift):
        ...
        dict =GIFT_EFFECTS[gift]
        self.change_affinity(dict[self.name])
...
class Game:
    ...
    def scene_xiaobai(self):
        ...
        if choice == '1':
            print("\n你轻松解出了那道算法题，小白眼中闪烁着崇拜的光芒。")
            print("小白：『太厉害了！能教教我吗？』你进入【小白线】！")
    ...
    def show_gift_menu(self):
        ...
        for  i in GIFT_EFFECTS.keys():
            print(i)
        choice = input('请选择你想送出的礼物🎁：')
        if choice not in GIFT_EFFECTS:
            print('\n送出的礼物不合法哦')
        self.current_target.give_gift(choice)
    

    def show_affinity_info(self):
        print(f'\n查看{self.current_target.name}好感度:{self.current_target.affinity}')
