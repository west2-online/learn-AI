...
如果你想了解和传统 AI 聊天机器人有关的东西，你可以看看作业 3。
...
虚拟伴侣需要采用 LangChain 搭建，并且需要有合理的上下文管理策略，尝试思考我们上一轮作业学习到的 RAG 可以在虚拟伴侣中负责什么功能。同时我们要求你集成一些基本工具。这些功能并使用 LangChain 并不难实现。

#### 作业要求 - 作业 3

- 使用 LangChain 搭建你的虚拟伴侣。
- 设计合理的上下文管理策略。
- 在项目中使用 RAG 实现某些功能。
- 设计一些简单的 tool 例如让你的虚拟伴侣看看你的文件夹下有哪些文件。
- （Bonus）给虚拟伴侣配置前后端。
- （Bonus）对模型进行微调。
- （不要求）运维部分
...
你将实现的虚拟伴侣需要基于 LangChain 来实现，如果你已经有后端的基本知识，那么你只需要阅读文档就可以很快上手。

这里提供两个实现方案，这两种方案在特定的情况下有自己的优势。
...
- 上下文管理
  ...
  你可以借助 LangChain 的相关功能将上下文管理集成到你的应用中。
