...
本考核遵循一个原则：从底层到高阶，而不直接使用高级框架。

### 底层实现

在入门阶段，我们限制使用高级框架，要求从零开始手动实现算法的组件。从损失函数到完整的反向传播链条，再到批量归一化等关键组件，都需要亲手实现。

这一过程能够确保你深刻理解数据在网络中的流动方式、梯度的计算过程，从而建立扎实的理论与实践基础。

### 理解

在底层实现阶段，通过要求向量化编程，培养高效的数据处理思维。在更高阶的考核中，即使允许使用 PyTorch，考核重点也从「实现」转向「理解」。

你需要将先前构建的模块（如 CNN）与新的复杂模型（如 RNN、Transformer）进行整合，通过可视化注意力权重、分析模型内部状态等方式，证明你不仅能够搭建系统，更能深入理解其内在机制。
